...
def train(epoch, global_step):
    ...
    train_loader = DataLoader(dataset = train_dataset, shuffle=True, num_workers=gpu_num, batch_size=gpu_per_batch, pin_memory=False)
    net.train()
...
if __name__ == "__main__":
    ...
    net = model.cuda()
    # net = torch.nn.DataParallel(net, list(range(gpu_num)))#.to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
    ...
    # optimizer =  torch.nn.DataParallel(optimizer, device_ids=None)
    # save_model(model, 0)
